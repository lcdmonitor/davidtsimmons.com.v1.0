...
namespace DavidSimmons.Authentication
{
    public class GraphHelper
    {
        public static string AcquireToken(string userObjectId)
        {
            ...
            AuthenticationContext authContext = new AuthenticationContext(ConfigHelper.Authority, new TokenStorageCache(userObjectId));
            AuthenticationResult result = authContext.AcquireToken(ConfigHelper.GraphResourceId, cred);
            return result.AccessToken;
        }

        public async static Task GetDirectoryObjects(List<string> objectIds, List<Group> groups, List<User> users, ClaimsIdentity claims)
        {
            List<DirectoryRole> roles = new List<DirectoryRole>();
            await GetDirectoryObjects(objectIds, groups, roles, users, claims);
        }

        public async static Task GetDirectoryObjects(List<string> objectIds, List<Group> groups, List<DirectoryRole> roles, ClaimsIdentity claims)
        {
            List<User> users = new List<User>();
            await GetDirectoryObjects(objectIds, groups, roles, users, claims);
        }

        public async static Task GetDirectoryObjects(List<string> objectIds, List<Group> groups, List<DirectoryRole> roles, List<User> users, ClaimsIdentity claims)
        {
            //NOte: you can use ClaimsPrincipal.Current to get the current
            string userObjectId = claims.FindFirst(Globals.ObjectIdClaimType).Value;
            ActiveDirectoryClient graphClient = new ActiveDirectoryClient(new Uri(ConfigHelper.GraphServiceRoot), () => { return Task.FromResult(AcquireToken(userObjectId)); });
